...
async searchWorlds(sort: string, tag: string, search: string, page: number) : Promise<Result<VRChatWorld[], string>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("search_worlds", { sort, tag, search, page }) };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
},
async createWorldInstance(worldId: string, instanceTypeStr: string, regionStr: string) : Promise<Result<null, string>> {
    try {
...
async createGroupInstance(worldId: string, groupId: string, instanceTypeStr: string, allowedRoles: string[] | null, regionStr: string, queueEnabled: boolean) : Promise<Result<null, string>> {
    try {
...
async migrateOldData(worldsPath: string, foldersPath: string, dontOverwrite: [boolean, boolean]) : Promise<Result<null, string>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("migrate_old_data", { worldsPath, foldersPath, dontOverwrite }) };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
...
export type VRChatWorld = { authorId: string; authorName: string; capacity: number; recommendedCapacity: number | null; created_at: string; favorites: number; visits: number | null; heat: number; id: string; imageUrl: string; name: string; popularity: number; publicationDate: string; releaseStatus: ReleaseStatus; tags: string[]; thumbnailImageUrl: string; unityPackages: UnityPackage[]; updated_at: string }
...
export type WorldDisplayData = { worldId: string; name: string; thumbnailUrl: string; authorName: string; favorites: number; lastUpdated: string; visits: number; dateAdded: string; platform: Platform; folders: string[] }
